...
export default function GateSettingsList({ gates, projectId, onChange }) {
  ...
  async function updateDescription(gate, description) {
    if (description === (gate.description || '')) return;
    await gatesApi.update(gate.id, { description });
    onChange();
  }
        ...
        <div key={gate.id}>
          <div className="flex items-center gap-2 p-3">
            ...
            <button
              ...
              onClick={() => setExpandedId(expandedId === gate.id ? null : gate.id)}
              aria-label={expandedId === gate.id ? 'Collapse gate' : 'Edit gate description'}
            >
              {expandedId === gate.id ? <ChevronDown size={15} /> : <ChevronRight size={15} />}
            </button>
            <input className="field flex-1 border-transparent px-2 py-1" defaultValue={gate.name} onBlur={(e) => rename(gate, e.target.value)} />
            {gate.status === 'CLOSED' && <span className="chip shrink-0 bg-slate-100 text-muted">Closed</span>}
          ...
          {expandedId === gate.id && (
            <div className="px-3 pb-3 pl-14">
              <label className="mb-1 block text-xs font-semibold uppercase tracking-wide text-muted">Description (optional)</label>
              <textarea
                className="field min-h-16"
                defaultValue={gate.description || ''}
                placeholder="What is this gate for?"
                onBlur={(e) => updateDescription(gate, e.target.value)}
              />
              {gate.status === 'CLOSED' && gate.closedReason && (
                <p className="mt-2 text-xs italic text-muted">Closed: &ldquo;{gate.closedReason}&rdquo;</p>
              )}
            </div>
          )}
        </div>
